...
- 2026-07: Domain purchased by user: bravelilystudio.com (EC2 + nginx deployment). Added /app/scripts/nginx-bravelily.conf (serves frontend build, proxies /api → 127.0.0.1:8001). Deploy steps: Elastic IP + A records for @ and www; on EC2 remove REACT_APP_BACKEND_URL from frontend/.env before yarn build (same-origin /api fallback handles routing); certbot for HTTPS.
- 2026-07: Fixed EC2 404 on form submit — backend now answers on BOTH /api/contact and /contact, so it works whether nginx proxy_pass keeps or strips the /api prefix (trailing-slash gotcha). nginx config carries a warning comment about it.
